/* Rina's 2048 UI v72 — Direction A gameplay/staging integration. */
(function () {
  "use strict";
  ...
  function mpMovementKeysMarkup() {
    var scheme = window.rinasSettings && window.rinasSettings.controlScheme === "wasd" ? "wasd" : "arrows";
    if (scheme === "wasd") {
      return '<span class="mp-guide-key-cluster wasd"><kbd>W</kbd><span><kbd>A</kbd><kbd>S</kbd><kbd>D</kbd></span></span>';
    }
    return '<span class="mp-guide-key-cluster arrows"><span><kbd>↑</kbd></span><span><kbd>←</kbd><kbd>↓</kbd><kbd>→</kbd></span></span>';
  }

  function mpFreeplayKeysMarkup() {
    var scheme = window.rinasSettings && window.rinasSettings.controlScheme === "wasd" ? "wasd" : "arrows";
    if (scheme === "wasd") {
      return '<span class="mp-fp-keygrid"><kbd>W</kbd><span><kbd>A</kbd><kbd>S</kbd><kbd>D</kbd></span></span>';
    }
    return '<span class="mp-fp-keygrid"><kbd>↑</kbd><span><kbd>←</kbd><kbd>↓</kbd><kbd>→</kbd></span></span>';
  }

  function mpScene(mode) {
    if (mode === "freeplay") {
      return '<div class="mp-freeplay-scene-v72">' +
        '<div class="mp-freeplay-board-v72">' + boardMarkup(FREEPLAY) + '</div>' +
        '<div class="mp-freeplay-controls-v72" aria-label="Movement controls and one-step Undo">' +
          '<div class="mp-fp-control-v72">' +
            '<b>MOVE</b>' + mpFreeplayKeysMarkup() + '<small>Play normally</small>' +
          '</div>' +
          '<div class="mp-fp-rule-v72" aria-hidden="true"></div>' +
          '<div class="mp-fp-control-v72 undo">' +
            '<b>UNDO</b><kbd class="mp-fp-z">Z</kbd><small>One step</small>' +
          '</div>' +
        '</div>' +
      '</div>';
    }
  ...
  function decorateRoomCode() {
    Array.prototype.forEach.call(document.querySelectorAll(".room-code-display"), function (display) {
      if (display.dataset.r71Decorated === "true") return;
      var code = String(display.textContent || "").replace(/\s+/g, "").trim().toUpperCase();
      if (!code) return;
      display.dataset.r71Decorated = "true";
      display.textContent = code;
      display.classList.add("r71-whole-room-code");
      display.setAttribute("title", "Click to copy room code");
  ...
  function decorateSoloPreviewControls() {
    ...
    caption.innerHTML = previewMovementKeysMarkup() +
      (undoEnabled ?
        '<span class="solo-preview-control undo"><span class="solo-preview-control-label">Undo</span><kbd>Z</kbd></span>' :
        '');
  }
